...
class Model:
    ...
    @data_changed
    def resampling(self,sampling):
        self.current["data"].resample(sampling)
        self.history.append(f"data.resample({sampling}")
        self.current["name"] += " (Resample)"
    ...
    @data_changed
    def apply_ica(self,exclude_comp):
        self.current["ica"].apply(self.current["data"],exclude=exclude_comp)
