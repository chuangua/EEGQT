...
class MainWindow(QMainWindow):
    ...
    def __init__(self, model: Model):
        ...
        self.setWindowTitle("MNELAB_lisha")
        ...
        self.actions["plot_ica_sources"] = plot_menu.addAction("Plot ICA &sources",self.plot_ica_sources)

        #lisha------------------
        # self.actions["plot_ica_properties"] = plot_menu.addAction("Plot ICA &properties",self.plot_ica_properties)
        self.actions["plot_ica_overlay"] = plot_menu.addAction("Plot ICA &overlay",self.plot_ica_overlay)
        #------------
        ...
        ###lisha's methods---------------------------------------------------------------------
        ERP_menu = self.menuBar().addMenu("&ERP")
        self.actions["Check_Bad"] = ERP_menu.addAction("&Check Bad Channle", self.CheckBad)
        self.actions["ERPtTest"] = ERP_menu.addAction("&ERP", self.ERPtTest)
        ERP_menu.addSeparator()
        self.actions["showcontact"] = ERP_menu.addAction("&Contact",self.showcontact)
        #-----------------------------------------------------------------------------------
        ...
        tobe_menu = self.menuBar().addMenu("&Want2Add")
        self.actions["addmore"] = tobe_menu.addAction("&Contact", self.addmore)
    ...
    def plot_ica_overlay(self):
        n_components = self.model.current["ica"].n_components
        components=list(range(n_components))
        types = ['comp']
        dialog = PickComponents(self, components, types)
        if dialog.exec():
            picks = [item.text() for item in dialog.names.selectedItems()]
            if set(components) == set(picks):
                return

        picks=[int(p) for p in picks]
        self.model.current["ica"].plot_overlay(inst=self.model.current["data"], exclude=picks)

    def resampling(self):
        dialog = SampleDialog(self)
        if dialog.exec():
            self.auto_duplicate()
            self.model.resampling(dialog.sampling)
            ...
            #         self,
            #         "Could not resampling data",
            #         str(e),
            #         traceback.format_exc(),
            #     )
            #     msgbox.show()

    # def plot_ica_properties(self):
    #     n_components = self.model.current["ica"].n_components
    #     components=set(range(n_components))
    #     types = ['comp']
    #     dialog = PickComponents(self, components, types)
    #     if dialog.exec():
    #         picks = [item.text() for item in dialog.names.selectedItems()]
    ...
    #
    #     picks=[int(p) for p in picks]
    #     self.model.current["ica"].plot_properties(inst=self.model.current["data"], picks=picks)
    #---------------------------------------------------------------------
    def plot_erds(self):
        """Plot ERDS maps."""
    ...
    def run_ica(self):
        ...
        if dialog.exec():
            ...
            ica = mne.preprocessing.ICA(method=method, fit_params=fit_params,n_components=n_components,random_state=7)
    ...
    def apply_ica(self):
        ...
        picks=[int(p) for p in picks]
        self.model.apply_ica(exclude_comp=picks)
        #-----------
    ...
    #lisha---------------------------------------------------------
    def CheckBad(self):
        ch_names=self.model.current["data"].ch_names
        data_ = self.model.current["data"].copy().get_data() * 1e6  # * 1e6 to transform to microvolt
        ...
        z_scores = 0.6745 * (mad_scores - np.nanmedian(mad_scores)) / scipy.stats.median_abs_deviation(mad_scores,
                                                                                                       scale=1)
        bad_dev = [ch_names[i] for i in np.where(np.abs(z_scores) >= 3.0)[0]]
        message=f"Bad Channel: {bad_dev}"
        dialog = HistoryDialog(self, message)
        dialog.exec()

    def showcontact(self):
        message=f"any problem with your contact\nLisha Li\nDepartment of Music AI and Information Technology, Central Conservatory of Music\nTsinghua Laboratory of Brain and Intelligence (THBI)\nemail: lsha.li@outlook.com\ntele: +86 17362975379"
        dialog = HistoryDialog(self, message)
        dialog.exec()
    def addmore(self):
        message=f"if you have any ideas about new functions, please contact me at EMAIL"
        dialog = HistoryDialog(self, message)
        dialog.exec()
    def ERPtTest(self):
        event_types = np.unique(self.model.current["events"][:, 2]).astype(str).tolist()
        dialog = ERPDialog(self, event_types)
        if dialog.exec():
            window = dialog.tmin.value()
            alpha = dialog.tmax.value()

            if dialog.baseline.isChecked():
                # baseline = dialog.a.value(), dialog.b.value()
                GaussianAlpha = dialog.a.value()
            else:
                GaussianAlpha = False

            try:
                eventid=str(dialog.selected_events[0])
                Epoch=self.model.current["data"][eventid]
                ...
                for k, ch in enumerate(ch_names):
                    ERPtrial = Epoch.copy().pick(ch).get_data()[:, 0, :]
                    EvokeData = Evoke.copy().pick(ch).get_data()[0]
                    EvokeDataTime = Evoke.times

                    plotxy = True if k == 28 else False
                    ax = fig.add_subplot(8, 4, k + 1)
                    ax = ERPtTestFunction(ERPtrial, EvokeData, EvokeDataTime, baseline=0, window=window,tAlpha=alpha,
                                  ax=ax, plotxy=plotxy, GaussianAlpha=GaussianAlpha)
                    ax.set_title(f'{ch}')
                fig.show()
...
def ERPtTestFunction(ERPtrial,EvokeData,EvokeDataTime,baseline,window,tAlpha,ax,plotxy,GaussianAlpha=0.1):
    n_trial, n_timepoint = ERPtrial.shape
    Pvalue=[]
    window=int(window)

    for t in range(n_timepoint):
        data = ERPtrial[:, t]
        if GaussianAlpha:#检查正态性
            _, p_value = stats.normaltest(data)
            if p_value < GaussianAlpha:
                print(f'time {t} do not fit gauss')

        _, p_value = stats.ttest_1samp(data, baseline)
        Pvalue.append(p_value)

    onset = []
    offset = []
    for i in range(len(Pvalue) - window):
        onsetflag = True
        ...
        if onsetflag:
            onset.append(i)
            offset.append(i + window)

    onsetdiff = np.diff(np.array(onset))
    idx = np.where(np.isin(onsetdiff, onsetdiff[onsetdiff != 1]))[0]

    if ax != None:
        ax.axvline(0, color="k", linestyle="--", alpha=0.5)
        ax.axhline(0, color="k", linestyle="--", alpha=0.5)
        ax.plot(EvokeDataTime, EvokeData, color="c", label="Evoked")
        if plotxy:
    ...
    if len(idx) != 0:
        for i in range(len(idx) + 1):
            ons = 0 if i == 0 else idx[i - 1] + 1
            endidx = -1 if i == len(idx) else idx[i]
            tstart = onset[ons]
            tend = max(onset[endidx], offset[ons])
            if ax != None:
                ax.fill_between(EvokeDataTime[tstart:tend], EvokeData[tstart:tend], 0, color='red', alpha=0.3)
